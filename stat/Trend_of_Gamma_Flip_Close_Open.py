...
# Plot the trend of Gamma Flip, Open, and Close prices with specified background color conditions
plt.figure(figsize=(14, 8))

# Plot with background colors based on conditions
for i in range(len(data)):
    if data['Open'][i] > data['Gamma Flip'][i] and data['Close'][i] < data['Gamma Flip'][i]:
        plt.axvspan(i - 0.5, i + 0.5, color='orange', alpha=0.3)
    elif data['Open'][i] > data['Gamma Flip'][i]:
        plt.axvspan(i - 0.5, i + 0.5, color='lightgreen', alpha=0.3)
    elif data['Open'][i] < data['Gamma Flip'][i] and data['Close'][i] > data['Gamma Flip'][i]:
        plt.axvspan(i - 0.5, i + 0.5, color='darkgreen', alpha=0.3)
    else:
        plt.axvspan(i - 0.5, i + 0.5, color='lightcoral', alpha=0.3)

# Plot lines
plt.plot(data['Date'], data['Gamma Flip'], marker='o', linestyle='-', color='g', label='Gamma Flip')
...
plt.title('Trend of Gamma Flip, Open, and Close Prices')
plt.xlabel('Date')
plt.ylabel('Price')
